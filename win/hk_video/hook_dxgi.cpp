...
bool HookD3D11::HookD3D(HMODULE d3d11_module) noexcept {
	...
	IDXGISwapChain_Present_ = reinterpret_cast<IDXGISWAPCHAIN_PRESENT>(tc::memory::GetVTableFunctionAddress(swap_chain, 8));

	// 13: swap_chain->ResizeBuffers
	IDXGISwapChain_ResizeBuffers_ = reinterpret_cast<IDXGISWAPCHAIN_RESIZEBUFFERS>(tc::memory::GetVTableFunctionAddress(swap_chain, 13));

	IDXGISwapChain1* swap_chain1;
	hr = swap_chain->QueryInterface(IID_PPV_ARGS(&swap_chain1));
	if (SUCCEEDED(hr)) {
		// 22: swap_chain1->Present1
		IDXGISwapChain1_Present1_ = reinterpret_cast<IDXGISWAPCHAIN1_PRESENT1>(tc::memory::GetVTableFunctionAddress(swap_chain1, 22));
		swap_chain1->Release();
	}
